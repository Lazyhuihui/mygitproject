...
//		config.checkOutTimeOut = Integer.valueOf(ConfigManager.instance
//				.getValue("db.config.checkOutTimeOut"));
//		config.dbName = ConfigManager.instance.getValue("db.config.dbName");
//		config.ip = ConfigManager.instance.getValue("db.config.ip");
//		config.maxIdleTime = Integer.valueOf(ConfigManager.instance
//				.getValue("db.config.maxIdleTime"));
//		config.maxPoolSize = Integer.valueOf(ConfigManager.instance
//				.getValue("db.config.maxPoolSize"));
//		config.minPoolSize = Integer.valueOf(ConfigManager.instance
//				.getValue("db.config.minPoolSize"));
//		config.driverClass = ConfigManager.instance
//				.getValue("db.config.driverClass");
...
//		config.testConnOnCheckin = Boolean.valueOf(ConfigManager.instance
//				.getValue("db.config.testConnOnCheckin"));
//		config.testSql = "SELECT * FROM T_ConnectTest;";
		
		config.checkOutTimeOut = Integer.valueOf("12000");
		config.dbName = Global.getConfig("jdbc.dbname");
		config.ip = Global.getConfig("jdbc.ip");;
		config.maxIdleTime = 1800;
		config.maxPoolSize = 20;
		config.minPoolSize = 1;
		config.driverClass = "com.mysql.jdbc.Driver";
		config.userName = Global.getConfig("jdbc.username");
		config.password = Global.getConfig("jdbc.password");;
		config.testConnOnCheckin = false;
		config.testSql = "SELECT * FROM T_ConnectTest;";
	...
	public int executeCommand(String sql, Object[] params) {
		...
		try {
			...
			//logger.error(String.format("exception in sql =  %s  %s", sql,
				//	params != null ? Arrays.toString(params) : ""), e);
			e.printStackTrace();
			return 0;
		} finally {
	...
	public <T> List<T> executeScalerList(String cmd, Object[] params,
			ResultObjectBuilder<T> builder) throws Throwable {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
